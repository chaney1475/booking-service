...
> **측정 조건**: WarmupRunner 완료(Caffeine 캐시 웜업 포함) 후 k6 실행. 임계값은 측정값 + 30% 버퍼.

| 시나리오 | 목적 | conn reset | 5xx | 핵심 수치 | threshold |
|----------|------|-----------|-----|-----------|-----------|
| s1 조회 폭주 | 1,000 VU 동시 checkout | 0건 | 0건 | p95=1.26s, p99=1.28s, errors=0 | p99<2s ✓ |
| s2 구매 폭주 | reserve.lua + saga | 0건 | 0건 | **PAID=9**, p95=967ms, p99=972ms | ≤10 ✓, p99<1.5s ✓ |
| s3 멱등성 스트레스 | 동일 idemKey 30회 | 0건 | 0건 | **mismatches=0**, PAID=10 | =0 ✓, p99<1.5s ✓ |
| s4 재고 초과판매 방지 | PAID ≤ 10 검증 | 0건 | 0건 | **PAID=9**, p95=677ms, p99=680ms | ≤10 ✓, p99<1s ✓ |
| s5 E2E 혼합 1,000 TPS | 조회+구매 혼합 | 0건 | 0건 | PAID=10, booking p99=706ms, checkout p99=1.42s | 전항목 ✓ |
...
- **s1 tail latency**: checkout p99 1.28s는 유저별 포인트 잔액 조회가 캐싱 불가한 DB 읽기이기 때문. 이벤트/옵션/상품 정보는 Caffeine 로컬 캐시(TTL 10s)로 처리. 포인트 조회를 분리하거나 Redis로 미러링하면 추가 개선 가능

### s1 checkout 성능 특성

WarmupRunner가 앱 기동 시 checkout 100회를 선제 호출해 Caffeine 캐시를 웜업한다. 이후 1,000 VU 폭주 기준 p99=1.28s이며, 병목은 유저 포인트 DB 읽기(커넥션 풀 경합)다. 구조적 개선 방향은 다음과 같다.

1. checkout에서 포인트 조회 분리 (상품·재고 API와 포인트 API 독립 호출)
2. `user_point`를 Redis에 미러링해 조회를 Redis로 전환 (차감 권위는 DB 유지)
