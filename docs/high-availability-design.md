...
```yaml
sliding-window-size: 10       # TIME_BASED — 최근 10초 기준
...
`ApplicationReadyEvent` 시점에 자기 자신에게 `/api/checkout`으로 150회(50 스레드 × 3라운드)를 전송한다.
...
- CLOSED → OPEN: 10초 윈도우 안에서 실패율 50% 초과 시 자동 전환 (TIME_BASED)
...
```
[Warmup] 시작 — 동시 50개 × 3라운드 = 150회
[Warmup] 완료 — 1243ms, 성공 150/150
```
