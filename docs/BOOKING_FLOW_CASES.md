...
→ 잔액 부족: 재고 반납 + 멱등 키 해제 후 400 INSUFFICIENT_POINT.  
  Order는 FAILED로 마킹됨 (pgApproved=false 분기에서 markFailed 호출).
...
→ confirm.lua 실패 (Redis 다운):  
   - CircuitBreaker fallback — 로그만 기록하고 흐름을 계속한다.  
   - purchased SET 미등록 + inflight TTL 대기. markPaid가 성공하면 200 PAID 반환.  
   ⚠️ 재구매 차단이 작동하지 않아 under-sell 가능. 정산 배치가 사후 정리.  
...
| 케이스 | 왜 위험한가 |
|---|---|
| Step 6 포인트 부족 | 재고 반납 + idem 해제 완료. Order는 FAILED로 마킹됨. |
