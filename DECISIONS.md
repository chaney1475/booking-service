...
Tomcat max-threads 200, min-spare 100, HikariCP maximum-pool-size 25, minimum-idle 10으로 설정했습니다.
...
HikariCP pool-size는 30이면 DB 커넥션 대기로 p95가 급등하고 50 이상이면 MySQL 자체가 병목이 됩니다. 2 CPU 파드 + MySQL 1 CPU 기준 25가 스위트스팟으로 확인됐습니다. Tomcat min-spare 100은 기동 직후 1대 장애 시 나머지 서버가 피크 TPS 전체를 혼자 수용할 수 있도록 잡은 값입니다.
